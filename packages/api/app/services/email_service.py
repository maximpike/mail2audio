...
class EmailService:
    ...
    def process_file(self, file_content: bytes) -> EmailBase:
        ...
        try:
            parsed_data = self.parser.parse_eml_file(
                file_content
            )  # do we decode in the service method or should we rather do this in the parser?
        except Exception as e:
            raise ValueError(f"Failed to parse email: {e!s}") from e
    ...
    def get_by_id(self, email_id: int) -> EmailBase | None:
